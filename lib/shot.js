 ...
 */
/** 上游代理是否可达：仅对 localhost/127.0.0.1 做本地端口探测；远端一律信任。 */
async function upstreamReachable(sh, upstream) {
	if (!upstream) return true;
	let u;
	try { u = new URL(upstream); } catch (e) { return false; }
	const host = u.hostname;
	if (host !== "127.0.0.1" && host !== "localhost" && host !== "::1") return true;
	const port = Number(u.port || (u.protocol === "https:" ? 443 : 80));
	return await portListening(sh, port);
}
...
export async function ensureProxy(sh, q) {
	...
	if (upstream && !(await upstreamReachable(sh, upstream))) {
		upstreamNote = "上游代理 " + upstream + " 不可达，已自动直连（流量仍经 mitmproxy 取证）";
		upstream = "";
	}
	...
	for (let i = 0; i < 25; i += 1) {
		if (await portListening(sh, port)) return { url, port, started: true, upstream: upstream || null, upstreamNote, index, ok: true };
		await sleep(300);
	}
	...
	return {
		...
		async execute(args, exec) {
			...
			return {
				...
				evidence,
				proxy: proxyInfo ? { url: proxyUrl, started: proxyInfo.started, upstream: proxyInfo.upstream || null, upstreamNote: proxyInfo.upstreamNote || null, ok: proxyInfo.ok, index: evidenceIndexPath() } : { url: null, note: "已禁用代理（无代理证据）" },
				warnings: proxyInfo && proxyInfo.upstreamNote ? [proxyInfo.upstreamNote] : [],
...
/**
 * 证据归档工具：把**任意来源**的证据（dsh-chrome 抓包/spill、截图、文本）
 * 复制/写入渗透证据目录，并回传标准 evidence 项。解决两件事：
 *   1) 外部产物（如 dsh-chrome 的 /var/folders spill、手工截图）在证据目录之外，
 *      Web 证据路由只服务 evidence 根目录（越界 403）；
 *   2) 临时文件会被系统清理，需要落地归档。
 * @param deps - 输出渲染器与提示文本。
 */
export function createEvidenceTool({ OUT, NOTE }) {
	return {
		name: "pentest_evidence_add",
		description:
			"把任意来源的证据归档进渗透证据目录并登记为可嵌入报告的 evidence 项：给 path 复制一个本地文件（如 dsh-chrome 的抓包结果/spill 文件、截图），或给 content 直接写入文本。返回的 evidence 项原样传给 pentest_add_finding / pentest_submit 的 evidence 字段即可在报告里呈现。" +
			NOTE,
		parameters: {
			type: "object",
			properties: {
				path: { type: "string", description: "要归档的本地文件绝对路径（与 content 二选一）。" },
				content: { type: "string", description: "或直接给文本内容（写成一个文件；与 path 二选一）。" },
				name: { type: "string", description: "归档文件名；默认取 path 的 basename，或 evidence-<时间戳>。<ext> 可省。" },
				kind: { type: "string", enum: ["screenshot", "http", "script", "file"], description: "证据类型（默认 file；截图 screenshot，原始包 http）。" },
				label: { type: "string", description: "证据标签（报告里显示）。" },
				findingId: { type: "string", description: "可选：归到该 finding 的子目录。" },
			},
			required: []
		},
		output: OUT,
		async execute(args) {
			const kind = ["screenshot", "http", "script", "file"].includes(String(args.kind)) ? String(args.kind) : "file";
			const findingId = String(args.findingId || "").trim();
			const dir = join(evidenceRoot(), findingId || "unfiled");
			mkdirSync(dir, { recursive: true });
			const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\..+/, "");
			const src = typeof args.path === "string" ? args.path.trim() : "";
			let name = typeof args.name === "string" && args.name.trim() !== "" ? args.name.trim() : "";
			if (name === "") name = src !== "" ? basename(src) : "evidence-" + stamp;
			const target = join(dir, name.replace(/[^\w.\u4e00-\u9fa5-]+/g, "_"));
			if (src !== "") {
				...
				return { ok: false, error: "需要 path 或 content 之一" };
			}
			const label = typeof args.label === "string" && args.label.trim() !== "" ? args.label.trim() : name;
			const evidence = [{ kind, path: target, label }];
			return { ok: true, saved: target, bytes: readFileSync(target).length, evidence, hint: "把 evidence 原样传给 pentest_add_finding / pentest_submit 的 evidence 字段。" };
		}
	};
}
function basename(p) {
